...
function Start-StateTransitions {
    if (-not $script:StateTransitionStarted) {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - STATE TRANSITIONS:"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
}
...
function Start-StateProcessing {
    ...
    # Log to file
    $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - ┌─ STATE: $($script:StatusIcons['Processing']) $stateIcon$StateName"
    $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - │  ├─ $depText"
    $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
}
...
function Write-StateCheck {
    ...
    # Log to file
    $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - │  ├─ Check: $($script:StatusIcons['Checking']) $CheckType check ($CheckDetails)"
    $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
}
...
function Write-StateCheckResult {
    ...
    if ($IsReady) {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [SUCCESS] - │  └─ Result: $status $resultText ($resultInfo)"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
    else {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - │  └─ Result: $status $resultText (proceeding with actions)"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
}
...
function Start-StateActions {
    ...
    # Log to file
    $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - │  ├─ Actions: $($script:StatusIcons['Executing']) EXECUTING"
    $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
}
...
function Start-StateAction {
    ...
    # Log to file
    $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - │  │  ├─ $ActionType`: $displayText"
    $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
...
function Complete-StateAction {
    ...
    if ($Success) {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [SUCCESS] - │  │  │  └─ Status: $status $statusText ($duration`s)"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
    else {
        Write-Host "│  │  │  └─ Status: $status $statusText ($duration`s)" -ForegroundColor $statusColor
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [ERROR] - │  │  │  └─ Status: $status $statusText ($duration`s)"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
}
...
function Complete-State {
    ...
    $resultColor = if ($Success) { "Green" } else { "Red" }
      if ($Success) {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [SUCCESS] - │  └─ Result: $status $resultText ($duration`s)"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [ERROR] - │  └─ Result: $status $resultText ($duration`s)"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - "
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
...
function Write-StateSummary {
    ...
    if ($sortedSuccessfulStates.Count -gt 0) {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [SUCCESS] - $($script:StatusIcons['Completed']) Successfully processed: $stateList"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
    ...
    if ($failedStates.Count -gt 0) {
        ...
        # Log to file
        $logMessage = "$(Get-Date -Format 'HH:mm:ss') [ERROR] - $($script:StatusIcons['Failed']) Failed: $stateList"
        $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
    }
    ...
    # Log to file
    $logMessage = "$(Get-Date -Format 'HH:mm:ss') [INFO] - ⏱️ Total time: $totalDuration`s"
    $logMessage | Out-File -FilePath $script:LogPath -Append -Encoding UTF8
...
Export-ModuleMember -Function Set-LogPath, Write-Log, Get-StateIcon, 
